<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
    ...
    $use->use_lib('db/tpl_slider');
    $use->use_lib('db/tpl_user_site');

    $use->use_lib('system/bootstrap/class_massage');
    ?>
    <h2 class="sub-header"><?= tpl_slider::slider() ?>s</h2>

    <div class="col-sm-12">
        <p id="status_massage"></p>
    </div>
    <div id="toolbar">
        <button id="add_new_btn" class="btn btn-success btn-xs">New <?= tpl_slider::slider() ?></button>
        </br>

    </div>
    <div class="table-responsive">
        <table id="table" data-toggle="table"
               data-url="<?= site_url('admin/' . tpl_slider::slider() . '/find_all_ajax') ?>"
            ...
            <tr>
                ...
                <th data-field="<?= tpl_slider::title() ?>" data-halign="center" data-sortable="true"> Title</th>
                <th data-field="<?= tpl_slider::description() ?>" data-halign="center" data-sortable="true">
                    description
                </th>

                <th data-field="<?= tpl_slider::image_path() ?>"
                    data-formatter="operate<?= tpl_slider::image_path() ?>"
                    data-events="events<?=tpl_slider::image_path() ?>"   data-width="10"
                ...
                <th data-field="<?= tpl_slider::status() ?>" data-halign="center" data-sortable="true"
                    data-formatter="operate<?= tpl_slider::status() ?>"
                    > Status
                </th>
                <th data-field="<?= tpl_slider::date_in() ?>" data-halign="center" data-sortable="true"> Last Date
                    Update
                </th>
                <th
                    data-field="operate"
...
<script type="text/javascript">
    ...
    function update_status(id, value) {
        $(document).ready(function () {
            $.post('<?= site_url('admin/'.tpl_slider::slider().'/update_status')?>',
                {
                    '<?=tpl_slider::slider().'_'.tpl_slider::id() ?>': id,
                    '<?=tpl_slider::slider().'_'.tpl_slider::status()?>': value
                }, function (result) {
                    var $table = $('#table');
...
<script type="text/javascript">
    ...
    window.operateEvents = {
        'click .remove': function (e, value, row, index) {
            if (confirm("Did you actually want to delete the College!")) {
                $.post('<?= site_url('admin/'.tpl_slider::slider().'/remove')?>', {'<?=tpl_slider::slider().'_'.tpl_slider::id()?>': row.<?=tpl_slider::id()?>}, function (result) {
        ...
        'click .update': function (e, value, row, index) {
            $('#update').modal('show');
            $('#<?=tpl_slider::slider().'_'.tpl_slider::id().'_update'?>').val(row.<?=tpl_slider::id()?>);
            $('#<?=tpl_slider::slider().'_'.tpl_slider::title().'_update'?>').val(row.<?=tpl_slider::title()?>);
            $('#<?=tpl_slider::slider().'_'.tpl_slider::description().'_update'?>').val(row.<?=tpl_slider::description()?>);
    ...
    $(document).ready(function () {
        ...
        $('#add_new').bootstrapValidator({
            ...
            fields: {
                '<?=tpl_slider::slider().'_'.tpl_slider::title()?>': {
                    validators: {notEmpty: {message: 'The field is required and can\'t be empty'}}
                },
                '<?=tpl_slider::slider().'_'.tpl_slider::description()?>': {
                    validators: {notEmpty: {message: 'The field is required and can\'t be empty'}}
                }
                ,
                image_new: {
            ...
            $("#add_new").on('submit', (function (e) {
                e.preventDefault();
                $.ajax({
                    url: "<?=site_url('admin/slider/insert')?>",
                    type: "POST"
        ...
        $('#update_form').bootstrapValidator({
            ...
            fields: {
                '<?=tpl_slider::slider().'_'.tpl_slider::title().'_update'?>': {
                    validators: {notEmpty: {message: 'The field is required and can\'t be empty'}}
                },
                '<?=tpl_slider::slider().'_'.tpl_slider::description().'_update'?>': {
                    validators: {notEmpty: {message: 'The field is required and can\'t be empty'}}
                },

                '<?=tpl_slider::slider().'_'.tpl_slider::id().'_update'?>': {validators: {notEmpty: {message: 'The field is required and can\'t be empty'}}}
            }
        }).on('success.form.bv', function (e) {
            e.preventDefault();
            var $form = $(e.target);
...
<script type="text/javascript">
    ...
    function operate<?= tpl_slider::image_path() ?>(value, row) {
        return ['<a class="update ml10" href="javascript:void(0)" title="Remove">',
            '<img  src="<?=site_url()?>' + value + '" style="width: 100%" />', '</a>'].join('');
    }
    window.events<?= tpl_slider::image_path() ?> = {
        'click .update': function (e, value, row, index) {

            $('#name_students').html(row.<?= tpl_slider::title() ?>);
            $('#image_students').attr('src', '<?=site_url()?>' + row.<?=tpl_slider::image_path() ?>);
            $('#update_image').modal('show');
            $('#<?=tpl_slider::slider().'_'.tpl_slider::id().'_update_image'?>').val(row.<?=tpl_slider::id()?>);
        }
    };
    $(document).ready(function () {
            ...
            <div class="modal-body">
                <form class="form" id="add_new" method="post" enctype="multipart/form-data"
                    ...
                    <div class="form-group">
                        <label for="Edit_NameCategory">Title <?= tpl_slider::slider() ?>: </label>
                        <input type="text" class="form-control"
                               id="<?= tpl_slider::slider() . '_' . tpl_slider::title() ?>"
                               name="<?= tpl_slider::slider() . '_' . tpl_slider::title() ?>"/>
                    </div>
                    <div class="form-group">
                        <label for="Edit_NameCategory">Title <?= tpl_slider::slider() ?>: </label>
                        <input type="text" class="form-control"
                               id="<?= tpl_slider::slider() . '_' . tpl_slider::description() ?>"
                               name="<?= tpl_slider::slider() . '_' . tpl_slider::description() ?>"/>
                    </div>
...
<div class="modal fade" id="update" style="background-color: rgba(60, 60, 60, 0.81);" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" style="width: 50%">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
                <h4>Update <?= tpl_slider::slider() ?>
                    <small id="TitlePostSmall"></small>
                </h4>
            </div>
            <div class="modal-body">
                <form class="form" id="update_form" method="post"
                      action="<?= site_url('admin/' . tpl_slider::slider() . '/update') ?>">

                    <div class="form-group">
                        <input type="hidden"
                               name="<?= tpl_slider::slider() . '_' . tpl_slider::id() . '_update' ?>"
                               id="<?= tpl_slider::slider() . '_' . tpl_slider::id() . '_update' ?>"
                               value=""/>
                    </div>

                    <div class="form-group">
                        <label for="">Name <?= tpl_slider::slider() ?> : </label>
                        <input type="text" class="form-control"
                               id="<?= tpl_slider::slider() . '_' . tpl_slider::title() . '_update' ?>"
                               name="<?= tpl_slider::slider() . '_' . tpl_slider::title() . '_update' ?>"/>
                    </div>
                    <div class="form-group">
                        <label for="">Name <?= tpl_slider::slider() ?> : </label>
                        <input type="text" class="form-control"
                               id="<?= tpl_slider::slider() . '_' . tpl_slider::description() . '_update' ?>"
                               name="<?= tpl_slider::slider() . '_' . tpl_slider::description() . '_update' ?>"/>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        $("#UploadImage").on('submit', (function (e) {
            e.preventDefault();
            $.ajax({
                url: "<?=site_url('admin/slider/update_image')?>",
                type: "POST"
                , data: new FormData(this), contentType: false,
                cache: false, processData: false,
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form role="search" class="" id="UploadImage" method="post" enctype="multipart/form-data" action="">
                    <input type="hidden" name="<?=tpl_slider::slider().'_'.tpl_slider::id().'_update_image'?>"
                           id="<?=tpl_slider::slider().'_'.tpl_slider::id().'_update_image'?>"/>
